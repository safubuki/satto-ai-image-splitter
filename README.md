...
*   **📲 レスポンシブデザイン**: スマートフォン、タブレット、デスクトップなど、あらゆる画面サイズに最適化されたUIを提供します。タッチ操作にも対応し、モバイルデバイスでも快適に利用できます。
...
1.  **APIキーの設定**:
    *   画面右上の設定アイコン（⚙️）をクリック/タップします。
    *   Google AI Studioで取得したAPIキーを入力します。
    *   *推奨*: モデルはデフォルトの `Gemini 3.1 Flash Lite` のままでOKです。

2.  **画像のアップロード**:
    *   トップ画面に画像をドラッグ＆ドロップするか、タップしてファイルを選択します。

3.  **分析と確認**:
    *   AIが自動的に画像を分析し、切り抜き範囲（緑色の枠）を表示します。
    *   下側（モバイル）または右側（デスクトップ）に切り抜かれた個別の画像が一覧表示されます。
...
本アプリケーションは以下のデバイス・画面サイズに対応しています：

*   **スマートフォン** (幅 ~640px): コンパクトなUI、タッチ操作に最適化されたボタンサイズ
*   **タブレット** (幅 640px~1024px): 中間サイズのUI
*   **デスクトップ** (幅 1024px~): フル機能のUI、2カラムレイアウト

デバイス検出には画面サイズだけでなく、タッチ対応の有無やユーザーエージェントも考慮しています。
