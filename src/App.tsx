        ...
        {!originalImage ? (
          <div className="py-12">
            ...
            {!apiKey && (
              <div
                onClick={() => setIsKeyModalOpen(true)}
                className="mt-6 mx-auto max-w-xl p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-center gap-3 cursor-pointer hover:bg-red-500/20 transition-all group animate-bounce-subtle"
              >
                <div className="p-2 bg-red-500/20 rounded-lg group-hover:scale-110 transition-transform">
                  <Settings className="w-5 h-5 text-red-400" />
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-red-400 text-sm">⚠️ APIキーが設定されていません</h3>
                  <p className="text-xs text-red-300/80">ここをクリックして、Google Gemini APIキーを設定してください。</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-medium text-gray-300">解析結果</h2>
            </div>
                  ...
                  {isProcessing && (
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center rounded-lg z-10">
                      <div className="text-center">
                        ...
                        <p className="text-mint-400 font-medium animate-pulse">Geminiで解析中...</p>
                      </div>
                    </div>
                  )}
                </div>
