  ...
  const handleAnalyze = async (file: File, base64: string) => {
    ...
    try {
      ...
      if (msg.includes("429") || msg.includes("quota")) {
        setError("Quota exceeded. Please try switching the Model in settings (e.g. to Gemini 1.5 Flash).");
      } else {
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col font-sans">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!originalImage ? (
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {error && (
              <div className="p-4 bg-red-950/50 border border-red-900/50 text-red-200 rounded-lg text-sm">
                Error: {error}
              </div>
            )}
            ...
            <div className="grid lg:grid-cols-2 gap-12">
              ...
              <div className="space-y-4">
                <p className="text-base text-gray-500 font-mono uppercase tracking-wider">Original / Overlay</p>
                  ...
                  {isProcessing && (
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center rounded-lg z-10">
                      <div className="text-center">
                        <div className="animate-spin w-8 h-8 border-2 border-mint-500 border-t-transparent rounded-full mx-auto mb-2" />
                        <p className="text-mint-400 font-medium animate-pulse">Analyzing with Gemini...</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Right: Results */}
              <div className="space-y-4">
                <p className="text-base text-gray-500 font-mono uppercase tracking-wider">Cropped Sub-Images</p>
                {cropResults.length > 0 ? (
                  <ResultGallery results={cropResults} />
                ) : (
                  <div className="h-full min-h-[400px] border-2 border-gray-800 border-dashed rounded-xl flex items-center justify-center text-gray-600 text-lg">
                    {isProcessing ? "Waiting for results..." : "No crops detected yet."}
                  </div>
                )}
              </div>
            </div>
      ...
      <footer className="py-6 border-t border-gray-900 mt-12">
        <div className="container mx-auto px-4 text-center text-gray-600 text-sm">
          <p>Powered by Google Gemini 2.5 Flash • Local-First Architecture</p>
        </div>
      </footer>
    </div>
  );
